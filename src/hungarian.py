...
def _find_augmenting_path(
    assignment: jnp.ndarray, adj_matrix: jnp.ndarray
    ...
    unassigned_workers = jnp.logical_not(jnp.any(assignment, axis=2, keepdims=True))
    unassigned_jobs = jnp.logical_not(jnp.any(assignment, axis=1, keepdims=True))

    unassigned_pairings = jnp.logical_and(
        adj_matrix, jnp.logical_not(assignment)
    ).astype(jnp.int32)
    ...
    init_workers = jnp.tile(worker_indices[jnp.newaxis, :, jnp.newaxis], (b, 1, 1))
...
def _improve_assignment(
    assignment: jnp.ndarray, state: Dict[str, jnp.ndarray]
) -> jnp.ndarray:
    ...
    def _backtrack_one_step(arg):
        ...
        update_indices = jnp.stack([batch_range, curr_worker_idx, curr_job_idx], axis=1)
    ...
    flip_matrix, _, _ = jax.lax.while_loop(
        _has_active_backtracks, _backtrack_one_step, (flip_matrix, active, curr_job_idx)
    )
...
def _maximum_bipartite_matching(
    ...
    state, assignment = jax.lax.while_loop(
        _has_new_jobs, _improve_assignment_and_find_new_path, (state, assignment)
    )
...
def _update_weights_using_cover(
    workers_cover: jnp.ndarray, jobs_cover: jnp.ndarray, weights: jnp.ndarray
) -> jnp.ndarray:
    ...
    uncovered_weights = jnp.where(covered, jnp.full_like(weights, max_value), weights)
    min_weight = jnp.min(uncovered_weights, axis=(-2, -1), keepdims=True)

    add_weight = jnp.where(
        double_covered, jnp.full_like(weights, min_weight), jnp.zeros_like(weights)
    )
    sub_weight = jnp.where(
        covered, jnp.zeros_like(weights), jnp.full_like(weights, min_weight)
    )

    return weights + add_weight - sub_weight


def hungarian_cover_matcher(weights: jnp.ndarray, eps: float = 1e-8) -> jnp.ndarray:
    ...
    def _cover_incomplete(arg):
        ...
        cover_sum = jnp.sum(workers_cover, dtype=jnp.int32) + jnp.sum(
            jobs_cover, dtype=jnp.int32
        )
